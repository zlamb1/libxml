...
xmlDebugRealloc(void *p, xmlSize size, void *ctx)
{
    ...
    assert(np != NULL);
    alloc = ( p == np ) ? xmlFindAlloc(&debugAllocator->table, p) : xmlMoveAlloc(&debugAllocator->table, p, np);
    debugAllocator->bytesAllocated += size;
...
xmlInitStdDebugAllocator(xmlAllocator **out)
{
    ...
    if ( ( error = xmlInitStdAllocator(&stdDebugAllocator->allocator) ) != LXML_ERR_NONE ) {
